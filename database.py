...
class Database:
    ...
    def add_lesson(self, user_id, day_name, subject, time, room="", teacher="",
                   reminder="🚫 Не напоминать", repeat="🚫 Без повтора"):
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
    ...
    def get_lessons_by_day(self, user_id, day_name):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def get_all_lessons(self, user_id):
        result = {}
    ...
    def get_days_with_lessons(self, user_id):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT DISTINCT day_name FROM lessons WHERE user_id = ?
            ORDER BY CASE day_name
                WHEN 'Понедельник' THEN 1 WHEN 'Вторник' THEN 2
                WHEN 'Среда' THEN 3 WHEN 'Четверг' THEN 4
                WHEN 'Пятница' THEN 5 WHEN 'Суббота' THEN 6
    ...
    def delete_lesson(self, user_id, lesson_id):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, subject, time FROM lessons WHERE id = ? AND user_id = ?", (lesson_id, user_id))
        row = cursor.fetchone()
        if row:
            result = dict(row)
    ...
    def update_lesson_field(self, lesson_id, field, value):
    ...
    def add_task(self, user_id, title, date, description="", time="",
                 cat_name="Без категории", cat_emoji="📌",
                 reminder="🚫 Не напоминать", repeat="🚫 Без повтора"):
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
    ...
    def get_tasks_by_date_range(self, user_id, start_date, end_date):
    ...
    def get_tasks_by_category(self, user_id, cat_name, cat_emoji):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute(f"""
            SELECT id, title, description, date, time,
    ...
    def delete_task(self, user_id, task_id):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, title, cat_emoji FROM tasks WHERE id = ? AND user_id = ?", (task_id, user_id))
        row = cursor.fetchone()
        if row:
            result = dict(row)
    ...
    def get_categories(self, user_id):
        ...
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, emoji FROM categories WHERE user_id = ? ORDER BY id", (user_id,))
        rows = [dict(row) for row in cursor.fetchall()]
        conn.close()
        return rows
    ...
    def add_category(self, user_id, name, emoji):
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("INSERT INTO categories (user_id, name, emoji) VALUES (?, ?, ?)", (user_id, name, emoji))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
    ...
    def delete_category(self, user_id, cat_id):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, emoji FROM categories WHERE id = ? AND user_id = ?", (cat_id, user_id))
        row = cursor.fetchone()
        if row:
            category = dict(row)
            cursor.execute("""
                UPDATE tasks SET cat_name = 'Без категории', cat_emoji = '📌'
    ...
    def update_category_field(self, cat_id, field, value):
        allowed = {"name", "emoji"}
        if field not in allowed:
            return False
        try:
            ...
            new_name = value if field == "name" else old["name"]
            new_emoji = value if field == "emoji" else old["emoji"]
            cursor.execute("""
                UPDATE tasks SET cat_name = ?, cat_emoji = ?
                WHERE user_id = ? AND cat_name = ? AND cat_emoji = ?
            """, (new_name, new_emoji, old["user_id"], old["name"], old["emoji"]))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
    ...
    def get_all_reminders(self, user_id):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT day_name, time, subject, reminder FROM lessons
        ...
        cursor.execute(f"""
            SELECT title, date, time, cat_emoji, reminder FROM tasks
            WHERE user_id = ? AND reminder != '🚫 Не напоминать'
            ORDER BY {self._date_expr('date')}, time
        """, (user_id,))
        for row in cursor.fetchall():
